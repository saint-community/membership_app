...
export default function FollowUpDetailView() {
  ...
  const formattedData = useMemo(() => {
    ...
    // Parse time to 12-hour format for display
        ...
        <View className="px-4 pt-4">
          ...
          <View className="mb-6 rounded-lg bg-white p-4 dark:bg-gray-800">
            ...
            <View className="mb-3">
              ...
              <Text className="text-base text-black dark:text-white">{formattedData.sessionDate}</Text>
            </View>

            <View className="mb-3">
              <Text className="mb-1 text-sm text-gray-400">Start Time</Text>
              <Text className="text-base text-black dark:text-white">{formattedData.startTime}</Text>
            </View>

            <View className="mb-3">
              <Text className="mb-1 text-sm text-gray-400">Location</Text>
              <Text className="text-base text-black dark:text-white">{formattedData.location}</Text>
            </View>
              ...
              <View className="mb-3">
                ...
                <View className="flex-row flex-wrap gap-2">
                  {formattedData.participants.map((participant, index) => (
                    <View
                      key={index}
                      className="rounded-full bg-[#FF007F]/10 px-3 py-1">
                      <Text className="text-sm text-[#FF007F]">{participant.name}</Text>
                    </View>
                  ))}
                </View>
              </View>
                ...
                return (
                  <View
                    ...
                    <View className="mb-3">
                      ...
                      <Text className="mb-1 text-sm text-gray-400">Material: {record.material}</Text>
                    ...
                    {record.members_taught.length > 0 && (
                      <View className="mb-3">
                        <Text className="mb-2 text-sm font-medium text-gray-400">Members Taught</Text>
                        <View className="flex-row flex-wrap gap-2">
